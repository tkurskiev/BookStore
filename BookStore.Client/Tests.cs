...
namespace BookStore.Client
{
    public class Tests
    {
        ...
        private static Command AddCommand(RootCommand rootCommand, string name, string? description, Action<string> handleParseResult)
        {
            var result = new Command(name, description);

            result.SetHandler(context =>
            {
                Console.WriteLine(context.ParseResult.ToString());
            });

            return result;
        }

        //static async Task ParsedResultHandler<T>(T parsedResult, Option<T> option)
        //{
        //    // TODO: че делать с результатом парсинга опции команды
        //}

        static async Task TestMethod3(params string[] args)
        {
            //var test = new CliBuilder("Client").AddCommand<string>(
            //    name: "get",
        ...
        static async Task TestMethod2(RootCommand rootCommand, params string[] args)
        {
            var testCommand = new Command("test", "Test command");

            var testOption = new Option<string>(
                "--testOption",
                "Test option for the TEST command");

            testCommand.AddOption(testOption);

            //var testParsedValue = testOption.Parse(args);

            var testValue = testCommand.Parse(args).GetValueForOption(testOption);
            Console.WriteLine($"Parsed test option value: {testValue}");
            //Console.WriteLine($"Parsed test option value: {testParsedValue.}");


        }

        static async Task TestMethod(params string[] args)
        {
